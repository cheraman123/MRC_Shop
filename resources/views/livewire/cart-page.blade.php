    ...
    <div class="flex flex-col md:flex-row gap-4">
      <div class="md:w-3/4">
        <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
          <table class="w-full">
            ...
            @foerelse ($cart_items as $item)
            <tr wire:key="{{$item['product_id']}}">
                <td class="py-4">
                  <div class="flex items-center">
                    <img class="h-16 w-16 mr-4" src="{{url('store', $item['image'])}}" alt="{{$item['name']}}">
                ...
                <td>
                  ...
                  <button wire:click='removeItem({{$item['product_it']}})' class="bg-slate-300 border-2 border-slate-400 rounded-lg px-3 py-1 hover:bg-red-500 hover:text-white hover:border-red-700">
                    <span wire:loading.remove wire:target='removeItem({{$item['product_it']}})'>Remove</span><span wire:loading wire:target="removeItem({{$item['product_it']}})">Removing..</span>
                  </button></td>
              </tr>
            @empty
      ...
      <div class="md:w-1/4">
        <div class="bg-white rounded-lg shadow-md p-6">
          ...
          @if($cart_items)
          <button href="/checkout" class="bg-blue-500 block text-center text-white py-2 px-4 rounded-lg mt-4 w-full">Checkout</button>
          @endif
        </div>
      </div>
    </div>
