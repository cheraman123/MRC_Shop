        ...
        <div class="w-full mb-8 md:w-1/2 md:mb-0" x-data="{ mainImage: '{{url('storage', $product->images)}}' }">
          <div class="sticky top-0 z-50 overflow-hidden ">
            ...
            <!-- @foreach ($product->images as $image)
              ...
              @endforeach -->
              <div class="w-1/2 p-2 sm:w-1/4" x-on:click="mainImage='{{url('storage',$product->images)}}'">
                <img src="{{url('storage',$product->images)}}" alt="{{$product->name}}" class="object-cover w-full lg:h-20 cursor-pointer hover:border hover:border-blue-500">
              </div>
          ...
          <!-- {!!Str::markdown($product->description)!!} -->
            ...
            <div class="flex flex-wrap items-center gap-4">
              <button wire:click="addToCart({{$product->id}})" class="w-full p-4 bg-blue-500 rounded-md lg:w-2/5 dark:text-gray-200 text-gray-50 hover:bg-blue-600 dark:bg-blue-500 dark:hover:bg-blue-700">
                <span wire:loading.remove wire:target='addToCart({{$product->id}})'>Add to cart <span wire:loading wire:target="addToCart({{$product->id}})">Adding...</span> </span>
              </button>
            </div>
          </div>
        </div>
